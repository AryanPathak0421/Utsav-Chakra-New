...
const Cart = () => {
  const { cartState, removeFromCart, updateQuantity, getTotalPrice, addToCart, clearCart } = useCart();
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: theme.semantic.text.secondary }}>
                {cartState.totalItems} item{cartState.totalItems !== 1 ? 's' : ''} • {Object.keys(vendorsByCategory).length} categor{Object.keys(vendorsByCategory).length !== 1 ? 'ies' : 'y'}
              </p>
            </div>
          </div>
          
          {/* Comparison Button - Responsive */}
